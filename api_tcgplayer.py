...
def clear_product_id(edition_id: str, debug: bool = False) -> None:
    ...
    wiping a bad match (mismatched tcgcsv sync, manual typo) so it can be
    rechecked from scratch. Leaves
...
def fetch_tcgcsv_products(group_id: str, debug: bool = False) -> list[dict]:
    ...
    together enough to match every edition in a set against its local record in
    one call."""
